const books = [
  {
    id: 24,
    img: '../imgs/happy-books/book-gallary/24lovejesus.jpg',
    thumbnail: '../../imgs/books/book-gallary/24lovejesus-tnail.jpg',
    title: '예수님과 사랑에 빠져라',
    author: '김일로',
    description:
      '"평생을 예수님의 사랑에 빠져 살아 온 김일로 선교사의 가슴 속 깊은 곳에서 우러나오는 진국같은 에세이. 이 책을 읽는 분들도 예수님과 사랑에 빠진 자가 될 수 있을 것입니다."',
    contents: [],
  },
  {
    id: 23,
    img: '../imgs/happy-books/book-gallary/23tree.jpg',
    thumbnail: '../../imgs/books/book-gallary/23tree-tnail.png',
    title: '소나무처럼',
    author: '박순옥',
    description: '',
    contents: [],
  },
  {
    id: 22,
    img: '../imgs/happy-books/book-gallary/22warmheart.jpg',
    thumbnail: '../../imgs/books/book-gallary/22warmheart-tnail.png',
    title: '설교를 돕기 위한 글모음: 마음이 따뜻한 사람',
    author: '양명환',
    description:
      '우리는 마음이 따뜻한 사람들입니다. 마음이 따뜻한 사람들끼리 서로의 마음을 열어 서로에게 행복을 전하는 행복의 천사가 되어야겠습니다.',
    contents: [],
  },
  {
    id: 21,
    img: '../imgs/happy-books/book-gallary/21jangrye.jpg',
    thumbnail: '../../imgs/books/book-gallary/21jangrye-tnail.jpg',
    title: '성경에서 말하는 장례',
    author: '김헌수',
    description:
      '국가공인 장례지도사의 자격과 교회 현장의 체험을 중심으로 성경적인 원리를 기초로 한 기독교 장례문화를 제시하다.',
    contents: [
      {
        chapter: '1. 서론',
        content: [
          '문제제기',
          '현대의 기독교 장례, 성경적인가?',
          '기독교 장례의 현실과 실례',
          '성경말씀은 실제이다',
        ],
      },
      {
        chapter: '2. 성경에서 말하는 죽음과 천국',
        content: [
          '사람은 왜 죽는가?',
          '그리스도인의 죽음은 무엇인가?',
          '그리스도인의 몸이란 어떤 것인가?',
          '천국은 어떤 곳인가?',
          '천국에서는 무엇을 하고 있나?',
          '천국에서의 모습은 어떠한가?',
          '천국은 어떻게 생겼나?',
          '천국의 혼인잔치 분위기는 어떠한가?',
          '천국은 누가 들어가나?',
          '왜 천국을 준비하셨나?',
          '해야 될 3가지 기본',
          '하지 말아야 될 3가지 기본',
        ],
      },
      {
        chapter: '3. 기독교적 용어 및 예식',
        content: ['바꿔야 할 용어 및 용품', '사용하지 말아야 할 용어 및 관습'],
      },
      {
        chapter: '4. 기독교 예식의 절차와 실제',
        content: [
          '천국환송 첫째 날',
          '천국환송 둘째 날',
          '천국환송 셋째 날',
          '추도 및 추모예배',
        ],
      },
      {
        chapter: '5. 결론',
        content: [
          '국가적인 장례추세',
          '기독교 예식의 비전과 제안',
          '맺음말과 선교의 장',
        ],
      },
    ],
  },
  {
    id: 20,
    img: '../imgs/happy-books/book-gallary/20wangdda.jpg',
    thumbnail: '../../imgs/books/book-gallary/20wangdda-tnail.jpg',
    title: '왕따가 왕이 된 이야기',
    author: '김기현',
    description: '',
    contents: [],
  },
  {
    id: 19,
    img: '../imgs/happy-books/book-gallary/19stillgood.jpg',
    thumbnail: '../../imgs/books/book-gallary/19stillgood-tnail.jpg',
    title: '설교를 돕기 위한 글모음: 여전히 좋은 사람',
    author: '양명환',
    description:
      '하루종일 주어도 금방 또 주고 싶어서 가슴에 미련마저 남게 하는 사람, 아깝지 않은 사랑을 주고 싶은 이가 바로 내 사랑 당신이랍니다. 세월이 흘러도 여전히 좋은 사람, 시간이 흘러 세상 속에 모든 것이 변해도 변하지 않는 마음을 알기에 하루도 빠짐없이 당신을 사랑합니다.',
    contents: [],
  },
  {
    id: 18,
    img: '../imgs/happy-books/book-gallary/18guwon.jpg',
    thumbnail: '../../imgs/books/book-gallary/18guwon-tnail.jpg',
    title: '새신자와 평신도 교육교재 - 구원의 은총',
    author: '양명환',
    description:
      '교육받지 않고는 일꾼이 될 수 없습니다. 교육받지 않고는 전도할 수 없습니다. 교육받지 않고는 행복해 질 수 없습니다. 누구든지 쉽게 접할 수 있고 읽기만 하면 교육이 되도록 편집했습니다.',
    contents: [],
  },
  {
    id: 17,
    img: '../imgs/happy-books/book-gallary/17whonow.jpg',
    thumbnail: '../../imgs/books/book-gallary/17whonow-tnail.jpg',
    title: '왜 지금 비전과 사명을 논해야 하는가?',
    author: '이남인',
    description: '',
    contents: [],
  },
  {
    id: 16,
    img: '../imgs/happy-books/book-gallary/16happyperson.jpg',
    thumbnail: '../../imgs/books/book-gallary/16happyperson-tnail.jpg',
    title: '설교를 돕기 위한 글모음: 가장 행복한 사람',
    author: '양명환',
    description:
      '못 생겼다고 짜증내지 마세요, 돈 없다고 짜증내지 마세요, 공부 못 한다고 화내지 마세요, 예쁘지 않다고 자신을 미워하지 마세요. 내가 지금 살아있지 않다면 이런 짜증도 낼 수 없답니다. 환하게 웃으면서 행복하게 하루를 보내 보세요. 나는 지금 현재 가장 행복한 사람입니다.',
    contents: [],
  },
  {
    id: 15,
    img: '../imgs/happy-books/book-gallary/15gido2.jpg',
    thumbnail: '../../imgs/books/book-gallary/15gido2-tnail.jpg',
    title: '새신자와 평신도가 필요로 하는 대표기도문(1, 2)',
    author: '양명환',
    description:
      '초신자들은 어떻게 기도해야 할지 방법을 잘 모르고 있습니다. 기독교 용어 자체가 생소하기 때문입니다. 주님 앞에 더 가까이 나아가는 방법은 하나님 앞에 기도하는 것입니다. 누구든지 쉽게 기도할 수 있도록 정리했습니다.',
    contents: [],
  },
  {
    id: 14,
    img: '../imgs/happy-books/book-gallary/14soul.jpg',
    thumbnail: '../../imgs/books/book-gallary/14soul-tnail.jpg',
    title: '당신의 영혼은 안녕하십니까?',
    author: '정수환',
    description: '',
    contents: [],
  },
  {
    id: 13,
    img: '../imgs/happy-books/book-gallary/13sinbaram.jpg',
    thumbnail: '../../imgs/books/book-gallary/13sinbaram-tnail.jpg',
    title: '신바람 박사와 실컷 웃어 보자',
    author: '황수관',
    description: '',
    contents: [],
  },
  {
    id: 12,
    img: '../imgs/happy-books/book-gallary/12counseling.jpg',
    thumbnail: '../../imgs/books/book-gallary/12counseling-tnail.jpg',
    title: '크리스천 카운셀링 네비게이터',
    author: '김영란',
    description: '',
    contents: [],
  },
  {
    id: 11,
    img: '../imgs/happy-books/book-gallary/11sweethome.jpg',
    thumbnail: '../../imgs/books/book-gallary/11sweethome-tnail.jpg',
    title: 'Home, Sweet 홈',
    author: '안정은',
    description:
      '안정은 목사는 칼빈대학교와 총신대학 신학대학원에서 목회학 석사과정 신학수업을 받고 현재는 서울 성석교회 부목사와 행복한가정연구소 부부행복학교 담당교수로 사역하고 있다. 기독교 TV-CTS, 국민일보와 기독교계 신문, 월간지에 여러 차례 소개되어 화제를 모으기도 했다. 이제는 부부들과 성도들의 신앙 성숙을 위하여 시작한 QT 나눔방이 전 교회 성도들로 확산되었고 교회와 가정을 말씀으로 건강하게 세워가는 일에도 좋은 모범이 되고 있다.',
    contents: [],
  },
  {
    id: 10,
    img: '../imgs/happy-books/book-gallary/10tree.jpg',
    thumbnail: '../../imgs/books/book-gallary/10tree-tnail.jpg',
    title: '생명나무 이야기',
    author: '김동일',
    description: '',
    contents: [],
  },
  {
    id: 9,
    img: '../imgs/happy-books/book-gallary/09choice.jpg',
    thumbnail: '../../imgs/books/book-gallary/09choice-tnail.jpg',
    title: '행복의 선택',
    author: '조주영',
    description: '',
    contents: [],
  },
  {
    id: 8,
    img: '../imgs/happy-books/book-gallary/08think.jpg',
    thumbnail: '../../imgs/books/book-gallary/08think-tnail.jpg',
    title: '조금만 생각을 바꾸면!',
    author: '박현식',
    description: '',
    contents: [],
  },
  {
    id: 7,
    img: '../imgs/happy-books/book-gallary/07miss.jpg',
    thumbnail: '../../imgs/books/book-gallary/07miss-tnail.jpg',
    title: '사무치는 그리움',
    author: '박명환',
    description:
      '지은이 박명환은 꾸밈도 가식도 없이 있는 그대로의 모습으로 살아가며 사람들을 너무 좋아해 누구든 상관없이 함께 어울려 세상 살아가는 이야기를 시간을 잊은 채 나누며, 한 바닥 넓이 만큼의 인생을 다양한 모습으로 살아간다는 그의 모습에서 참 자유인의 모습을 보게 된다. 노동자로, 빈민들을 위한 운동가로, 교사로, 새마을 지도자로, 교수로 폭넓은 삶을 살아온 그의 마지막 소원은 진실한 신앙인으로 영원히 기억될 사람이 되는 것이다.',
    contents: [],
  },
  {
    id: 6,
    img: '../imgs/happy-books/book-gallary/06metdol.jpg',
    thumbnail: '../../imgs/books/book-gallary/06metdol-tnail.jpg',
    title: '물맷돌 성경공부',
    author: '정수환',
    description:
      '학생회, 청년회 등의 성경공부교재로 적합합니다, 성도들이 개인적으로 공부하기에 적당합니다, 목회자가 설교준비를 하기에 적당합니다.',
    contents: [],
  },
  {
    id: 5,
    img: '../imgs/happy-books/book-gallary/05yechan.jpg',
    thumbnail: '../../imgs/books/book-gallary/05yechan-tnail.jpg',
    title: '웃음예찬',
    author: '조주영, 오현숙',
    description: '',
    contents: [
      {
        chapter: '1. 명사들의 웃음찬미',
        content: [
          '웃음예찬',
          '서로 미소를 지으십시오',
          '알랭의 웃음학',
          '웃음은 사랑의 표현',
          '참된 웃음',
          '웃음봉사찬미',
          '성공이란',
          '웃음에 관한 명언들',
        ],
      },
      {
        chapter: '2. 노래속의 웃음찬미',
        content: [
          '웃어',
          '웃어요',
          '활짝 웃어요',
          '스마일 스마일 스마일',
          '웃음치료 노래',
        ],
      },
      {
        chapter: '3. 연구결과에 나타난 웃음찬미',
        content: [
          '웃음을 알자',
          '웃음에 대한 이론',
          '웃음의 종류',
          '웃음의 건강증진 효과',
          '웃음의 효과와 관련된 최근 보도자료',
        ],
      },
    ],
  },
  {
    id: 4,
    img: '../imgs/happy-books/book-gallary/04imji.jpg',
    thumbnail: '../../imgs/books/book-gallary/04imji-tnail.jpg',
    title: '목회임지! 무엇이든지 물어보세요',
    author: '박현식',
    description:
      '한국교회 목회자들이 직면하고 있는 가장 시급한 문제는 목회임지의 문제이다. 현재 한국교회에서 목회하고 있는 목회자들 중 임지를 이동하고 싶은 마음이 있다고 생각하는 분들이 70% 이상 된다는 통계조사자료가 나와 있다. 현실이 이와 같은데, 무임목회자가 수십만 명이 넘는 한국교회의 상황 속에서 -한국 교회 목회임지의 현실, 목회 임지연결 성사의 구체적 성공사례, 목회임지 연결 시 실패하는 사례, 목회임지에 대해 목사님들이 질문하는 문제에 대한 구체적인 주제-를 다룬 책은 한국에서 단 한 권도 출간된 적이 없다. 아무쪼록 이 책이 목회임지 문제로 고민하고 방황하는 한국교회 모든 목회자들에게 분명한 갈 길을 제시해주는 나침반의 역할이 되었으면 하는 소박한 바람을 필자는 가지고 있다. (프롤로그 중에서)',
    contents: [],
  },
  {
    id: 3,
    img: '../imgs/happy-books/book-gallary/03sinbaram.jpg',
    thumbnail: '../../imgs/books/book-gallary/03sinbaram.jpg',
    title: '황수관 박사와 실컷 웃어보자',
    author: '황수관',
    description:
      '하루에 한번만 웃으면 수명이 이틀동안 연장된다고 한다. 요즘 의학계에서는 웃음치료에 대한 보고가 들어오고 있다. 웃음치료가 최고의 치료라는 것이 입증되고 있다. 웃음은 마음의 치료제요 몸의 미용제라고 한다.',
    contents: [],
  },
  {
    id: 2,
    img: '../imgs/happy-books/book-gallary/02stairs.jpg',
    thumbnail: '../../imgs/books/book-gallary/02stairs-tnail.jpg',
    title: '행복의 계단',
    author: '조주영',
    description: `책 한권을 읽는다는 것은 또 하나의 인생과의 만남을 의미합니다. 조주영 교수의 "행복의 계단"을 읽으면서 저자의 아름답고 깊은 마음, 다른 사람과 더불어 살아가려는 마음, 인생의 기로에 서있는 청소년에게 향하는 사랑의 마음, 행복한 생활인의 마음과 만나 출간의 기쁨과 더불어 행복한 시간을 가졌습니다.
    많은 이들이 "행복의 계단"을 통해 행복해졌으면 하는 바람도 가져보았습니다. (김영희 교수)`,
    contents: [
      {
        chapter: '1. 내면의 소리에 귀 기울이기',
        content: [
          '1) 만약 청소년이 분재와 같은 모습으로 살아간다면?',
          '2) 어느 청소년이 전하는 내면의 소리',
          '3) 부지불식간에 일어나는 바람직한 행동형성의 오류 이야기',
          '4) 문제를 바라보는 관점이 중요하다!',
          "5) '최적의 좌절경험'이 필요하다",
          '6) 아픔은 묻어두면 더 큰 화근이 된다',
          '7) 자아개념과 자아존중감',
          '8) 자신을 얼마나 좋아하고 신뢰하는가?',
        ],
      },
      {
        chapter: '2. 다름과 다양성 수용하기',
        content: [
          '1) 청소년, 그들의 이야기를 다시 만들어보게 하자',
          '2) 청소년과의 효율적 상호작용을 위해 필요한 것',
          '3) 성격이해교실 운영 소고',
          '4) 성격이해교실 운영 후기',
          '5) "아빠는 재방송, 선생님은 생방송"이라고?!',
          '6) 아들과 나눈 이색대화',
          '7) 아들아! 진정 소중한 것부터 먼저 하렴',
        ],
      },
      {
        chapter: '3. 이리 기웃, 저리 기웃 (자신에게 솔직해지기)',
        content: [
          '1) 솔직한 성, 건강한 성, 아름다운 성 이야기',
          '2) 전성(全性)교육과 예비부모교육의 필요성',
          '3) 태교지수 높이기',
          '4) 성교육은 인간관계교육',
          '5) 자녀 성교육',
          '6) 성교육을 위한 구체적인 방법',
          '7) 남자다움, 여자다움보다는 인간다움이 필요하다!',
        ],
      },
      {
        chapter: '웃음 플러스! 행복 플러스!',
        content: [
          '1) 웃음에는 뭔가 특별한 것이 있다!',
          '2) 지금 웃고 있습니까?',
          '3) 웃으면 얼굴표정이 예뻐진다!',
          '4) 나의 미소미용지수는 몇점일까?',
          '5) 억지로라도 웃으면 행복해진다',
          '6) 웃음 건강보험',
          '7) 생활 속의 웃음 치료',
          '8) 내 인생의 멘토, 웃음치료',
        ],
      },
      {
        chapter: '5. 행복지수 업그레이드',
        content: [
          '1) 커리어플래닝을 통한 자신의 가치실현하기',
          '2) 인생의 진정한 가치가 무엇인가?',
          '3) 인생의 진정한 멘토를 만나세요',
          '4) 인생 마라톤에서 정말 중요한 것은 완주하는 것',
          '5) 마음 근육도 신체 근육처럼 강화해야 한다',
          '6) 좋은 습관으로 행복지수 높이기',
          '7) 좀 더 행복해지기',
        ],
      },
    ],
  },
  {
    id: 1,
    img: '../imgs/happy-books/book-gallary/01bubuschool.jpg',
    thumbnail: '../../imgs/books/book-gallary/01bubuschool-tnail.jpg',
    title: '성숙한 가정을 위한 행복한부부학교 워크북',
    author: '김병훈',
    description:
      '그동안 저자가 오랫동안 부부세미나를 인도하면서 연구하고 경험했던 귀중한 자료를 바탕으로 교회나 집단뿐만 아니라 부부가 가정에서도 쉽게 사용할 수 있도록 제작되었다.',
    contents: [
      {
        chapter: '제1강 행복한 가정, 그 포기할 수 없는 이상',
        content: ['가정의 원리를 알면 행복이 보인다'],
      },
      {
        chapter: '제2강 남녀의 성차와 부부의 역할',
        content: ['나와는 너무나 딴판인 배우자 이해하기'],
      },
      {
        chapter: '제3강 에니어그램을 통한 자아분석',
        content: ['나를 알고 상대를 알면 백전백승(?)'],
      },
      {
        chapter: '제4강 부부갈등의 원인과 해결',
        content: ['한지붕 밑에 살면서 웬수가 따로 없어요'],
      },
      {
        chapter: '제5강 성공적인 부부대화의 기술',
        content: ['댁의 부부는 말이 잘 통하십니까?'],
      },
      {
        chapter: '제6강 마르지 않는 사랑의 샘, 부부의 성생활',
        content: ['여보, 우리 뜨겁게 사랑해 볼까요?'],
      },
      {
        chapter: '제7강 결혼과 영성',
        content: ['하나님이 기뻐하시는 가정 세우기'],
      },
      {
        chapter: '제8강 앙코르 결혼식',
        content: ['하나님이 기뻐하시는 가정 세우기'],
      },
    ],
  },
]
